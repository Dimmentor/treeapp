...
Синхронная (def)	Асинхронная (async def)
Блокирует поток при выполнении (если внутри есть I/O-операции).	Не блокирует поток (использует await).
Подходит для CPU-интенсивных задач.	Подходит для I/O-операций (запросы к БД, API).
Может замедлять работу при большом количестве запросов.	Эффективно работает с тысячами одновременных запросов.
